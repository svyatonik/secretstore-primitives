...
pub trait Connection<Message: Serialize + DeserializeOwned>: Send + Sync {
...
pub trait ConnectionManager<Message: Serialize + DeserializeOwned>: 'static + Send + Sync {
	/// Returns shared reference to connections provider.
	fn provider(&self) -> Arc<dyn ConnectionProvider<Message>>;
...
pub trait ConnectionProvider<Message: Serialize + DeserializeOwned>: Send + Sync {
	...
	fn connection(&self, node: &KeyServerId) -> Option<Arc<dyn Connection<Message>>>;
}
